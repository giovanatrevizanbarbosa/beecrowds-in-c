...
int main(){
    int alturaPulo, qntdCanos;
    int it;

    scanf("%d %d", &alturaPulo, &qntdCanos);

    int alturaCano[qntdCanos];
    for (it = 0; it < qntdCanos; it++){
        scanf("%d", &alturaCano[it]);
    }
    
    for (it = 0; it < qntdCanos-1; it++){
        int dif = alturaCano[it + 1] - alturaCano[it];
        if(dif > alturaPulo || dif < -alturaPulo){
            printf("GAME OVER\n");
            return 0;
        }
    }
